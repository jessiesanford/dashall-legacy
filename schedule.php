	...
	<div class="">
		...
		<?php 
			...
			else 
			{
				...
				$sql = mysql_query("
					SELECT * FROM drivers 
					INNER JOIN users ON drivers.user = users.user_id 
					WHERE drivers.user = ". $_SESSION['user_id'] ."
					LIMIT 1
				");
				$driver_data = mysql_fetch_assoc($sql);
				...
				echo '
					...
					<form id="driver_settings" method="POST">
						...
						<label class="label_boxed block push_bottom">
							<input type="checkbox" class="checkbox" name="notify_orders" '. (($driver_data['notify_orders'] == 1)?'checked':" ") .' /> <strong>Notify me of any extra orders that need to be delivered.</strong>
						</label>
					</form>
						...
						for ($i = 0; $i < 7; $i++)
						{
							...
							if ($i == 5 || $i == 6)
							{
								$shifts = array(
									...
									array($week[$i]->setTime(21, 00)->format('Y-m-d H:i:s'), $week[$i]->setTime(01, 00)->modify('+1 day')->format('Y-m-d H:i:s'))
								);
							}
							else 
							{
